...
Mushborg device
Shield: ESP8266-S
Board: ESP8266 Relay LC Technology
Ver:.1 pre
Author: Glitched
Date: 8-01-26
...
MQTT Command
mushborg/COD_DEVICE/edit {"factory_reset":1} for factory reset
mushborg/COD_DEVICE/switch {"relay":0-1} turn off-on relay

NOTE :
Use 9600 baud rate for serial communication

And use this command for open end close relays, wire ->C NO->
byte turnOn[] = {0xA0, 0x01, 0x01, 0xA2};
byte turnOff[] = {0xA0, 0x01, 0x00, 0xA1};
...
void printConfig() {
  //loadConfig();
  Serial.println("---- CONFIG ESP RELAY----");
...
String htmlPage(String nets) {
  ...
  return 
    ...
    "<h3>Mushborg</h3>" 
    "<h4>ESP8266 relay 1 canale setup</h3>"
    "<form action='/save'>"
...
void onMqttMessage(char* topic, char* payload, 
  ...
  // Creiamo una stringa per il confronto
  ...
  // Controllo SWITCH
  else if (top == base + "/switch") {
    StaticJsonDocument<128> doc;
    if (deserializeJson(doc, payload, len) == DeserializationError::Ok) {
      int value = doc["relay"] | -1; // -1 se non esiste
      if (value != -1) {
        Serial.printf("Relay impostato a: %d\n", value);
        setRelay(value); // Assicurati che setRelay esista
      }
    }
  }
}
...
void onMqttConnect(bool) {
  ...
  mqtt.subscribe((base + "/switch").c_str(), 1);
  Serial.println("Subscribed to /edit and /switch");
}
